...
from auth_service.src.services.auth import get_auth_service
...
async def test_login(client: AsyncClient, fake_auth_service: AsyncMock, fake_user_service: AsyncMock):
    ...
    mock_user.check_password = AsyncMock(return_value=True) # Более явный мок
...
async def test_refresh_token_success(
    client: AsyncClient, fake_auth_service: AsyncMock, fake_user_service: AsyncMock
):
    # Arrange
    user_id = uuid.uuid4()
    mock_user = User(id=user_id, login="test", email="test@test.com", password="pw")
    
    # Мокируем сервисы для корректной работы обновленного метода refresh
    fake_auth_service.get_user_from_token.return_value = mock_user
    fake_auth_service.redis.get.return_value = "valid_refresh_token" # Мокируем Redis
    ...
    fake_auth_service.get_user_from_token.assert_awaited_once_with("valid_refresh_token", fake_user_service)
